use <CylinderDispenser_Rail_Floor.scad>
use <CylinderDispenser_Rail_Wall.scad>
...
module CylinderDispenser_Rail(floorDimensions, railWall, wallDimensions){
	union(){
		CylinderDispenser_Rail_Floor(floorDimensions);
		if(railWall){
			translate([0, 0, floorDimensions.z])
			CylinderDispenser_Rail_Wall(wallDimensions);
		}
	}
}

CylinderDispenser_Rail(floorDimensions, railWall, wallDimensions);